...
#Region "Former Ratio Calculation Product"
    ...
    Public Function K201_FormerRatioCalculationProduct(ByRef preactorComObject As PreactorObj, ByRef pespComObject As Object) As Integer
        ...
        Do
            If (planningboard.GetOperationLocateState(y)) Then
                ...
                If intOperationNumber = 10 Then
                    ...
                    '' Dim orId As Integer = preactor.ReadFieldInt("Orders", "Order ID", y)
        ...
        Else
            ...
            ''============================Main Table============================
            Dim order_tb As DataTable = dt_s.DefaultView.ToTable(True, "OrderNo", "Product", "ProductQuantity", "OrderStartDate", "AvilableFormer", "FormersPerPlatoon", "OrderResourceRate", "OrderId", "SecondaryConstraintQty")
            ...
            ElseIf line_Type = "P" Then
                formerAvailability("#") = "Platoon Availability"
            Else
            ...
            ''==============If Platune only this will execute=================
            If line_Type = "P" Then
                rowId_ = 130
            ...
            For Each formDet As DataRow In tbl_formerDetailsHeader.Rows
                ...
                For Each corpro As DataRow In order_tb.Rows
                    ...
                    If qtyRowValue = order_ Then
                        ...
                        Dim coProQty As String = corpro("ProductQuantity").ToString()
                        formDet(sizeColumn) = coProQty
                    End If
                Next
                ...
                If rowValue = "Total" Then
                    ...
                    Next
                    formDet("Total") = Math.Round(totalOrder, 0)
                ...
                If fdRowId = 50 Then
                    For Each corpro As DataRow In Product_dt.Rows
                        ...
                        For Each order As DataRow In order_tb.Rows
                            ...
                            If ordCorProduct = corProduct Then
                                formDet(corProduct) = CDec(order("AvilableFormer").ToString())
                                totalFormer = totalFormer + CDec(order("AvilableFormer").ToString())
                            End If
                        Next
                    Next
                End If
                ...
                If fdRowId = 105 Then
                    formDet("Total") = productResourceRate
                End If
                'Former %
                If fdRowId = 110 Then
                ...
                If fdRowId = 115 Then
                    ...
                    For Each corpro As DataRow In Product_dt.Rows
                        ...
                        For Each order As DataRow In order_tb.Rows
                            Dim ordCorProduct As String = order("Product").ToString()
                            If ordCorProduct = corProduct Then
                                formDet(corProduct) = CDec(order("SecondaryConstraintQty").ToString())
                                totalFormersAssign = totalFormersAssign + CDec(order("SecondaryConstraintQty").ToString())
                            End If
                        Next
                    Next
                    formDet("Total") = totalFormersAssign
                End If

                'Former Balance
                If fdRowId = 120 Then
                    Dim AvbFormer As DataRow = tbl_formerDetailsHeader.Rows(CInt(order_tb.Rows.Count) + 2)
                    Dim ReqFormer As DataRow = tbl_formerDetailsHeader.Rows(CInt(order_tb.Rows.Count) + 5)
                    ...
                    For Each pro As DataRow In Product_dt.Rows
                        ...
                        For Each order As DataRow In order_tb.Rows
                            ...
                            If product = sProduct Then
                                formDet(sProduct) = Math.Round((CDec(precentage(sProduct).ToString()) / 100 * productResourceRate), 2)
                            End If
                        Next
                    Next
                End If
                ...
                If line_Type = "P" Then
                    ...
                    If fdRowId = 132 Then
                        Dim formersPerPlatoon As DataRow = tbl_formerDetailsHeader.Rows(CInt(order_tb.Rows.Count) + 8)
                        Dim platoonRequrment As DataRow = tbl_formerDetailsHeader.Rows(CInt(order_tb.Rows.Count) + 5)
                    ...
                    If fdRowId = 135 Then
                        Dim glovPair As DataRow = tbl_formerDetailsHeader.Rows(CInt(order_tb.Rows.Count) + 9)
                    ...
                    If fdRowId = 136 Then
                        ''Total %
                        Dim TotalPer As DataRow = tbl_formerDetailsHeader.Rows(CInt(order_tb.Rows.Count) + 2)
                        ''Glove Ratio
                        Dim glovRate As DataRow = tbl_formerDetailsHeader.Rows(CInt(order_tb.Rows.Count) + 10)
                    ...
                    If fdRowId = 140 Then
                        ''Total %
                        Dim platoonP As DataRow = tbl_formerDetailsHeader.Rows(CInt(order_tb.Rows.Count) + 5)
                        For Each corpro As DataRow In Product_dt.Rows
                            Dim product As String = corpro("Product").ToString()
                            formDet(product) = platoonP(product).ToString()
                        Next
                        Dim glratdiff As DataRow = tbl_formerDetailsHeader.Rows(CInt(order_tb.Rows.Count) + 11)
                        ...
                        Dim glratdiff_ As DataRow = tbl_formerDetailsHeader.Rows(CInt(order_tb.Rows.Count) + 5)
                        Dim glratdiffRate As DataRow = tbl_formerDetailsHeader.Rows(CInt(order_tb.Rows.Count) + 11)
                    ...
                    If fdRowId = 145 Then
                        ...
                        Dim formersPerPlatoon As DataRow = tbl_formerDetailsHeader.Rows(CInt(order_tb.Rows.Count) + 8)
                        Dim platoonRequrment As DataRow = tbl_formerDetailsHeader.Rows(CInt(order_tb.Rows.Count) + 12)
                    ...
                    If fdRowId = 155 Then
                        ''Adjusted Total %
                        Dim TotalPer As DataRow = tbl_formerDetailsHeader.Rows(CInt(order_tb.Rows.Count) + 13)
                        ''Adjusted Glove Ratio
                        Dim glovRate As DataRow = tbl_formerDetailsHeader.Rows(CInt(order_tb.Rows.Count) + 2)
                        For Each corpro As DataRow In Product_dt.Rows
                            Dim corProduct As String = corpro("Product").ToString()
                            formDet(corProduct) = Math.Round(CDec(TotalPer(corProduct).ToString()) - CDec(glovRate(corProduct).ToString()), 2)
                                ...
                                If orderRecordNum > 0 Then
                                    If "Production" = CStr(preactor.ReadFieldString("Orders", "Operation Name", orderRecordNum)) Then
                                        Try
                                            ...
                                            Dim sequance As Integer = CInt(K201_GetOrderResourceDataSequance(connetionString, orderId, int_Resource))
                                            ...
                                            Dim isSameProduct As Integer = CInt(K201_GetPreviousJobIsSameProductCode(connetionString, order_No, op_No, resource, startTime, productCode))
...
#End Region
    ...
    Public Function K201_GetOrderResourceDataSequance(ByRef connetionString As String, ByRef orderId As Integer, ByRef resourceId As Integer) As Integer
        ...
        Try
            ...
            connection.Close()
        Catch ex As Exception
            MsgBox("Resource Sequence Error",, "Error")
        Finally
    ...
    Public Function K201_GetPreviousJobIsSameProductCode(ByRef connetionString As String, ByRef orderNo As String, ByRef opNo As Integer, resourceId As Integer, startTime As DateTime, productCode As String) As Integer
        ...
        Try
            ...
            command.Connection = connection
            command.CommandType = CommandType.StoredProcedure
            command.CommandText = "K201_GetPreviousJobIsSameProductCode_Sp"
            command.CommandTimeout = 600
